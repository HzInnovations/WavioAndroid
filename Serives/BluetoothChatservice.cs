...
namespace Wavio.Serives
{
    class BluetoothActivityService
    {
        // Debugging
        private const string TAG = "BluetoothActivityService";
        private const bool Debug = true;

        // Name for the SDP record when creating server socket
        private const string NAME = "BluetoothActivity";

        // Unique UUID for this application
        private static UUID MY_UUID = UUID.FromString("4d91d212-f14a-11e5-9ce9-5e5517507c66");
        ...
        public void Connected(BluetoothSocket socket, BluetoothDevice device)
        {
            ...
            var msg = _handler.ObtainMessage(BluetoothActivity.MESSAGE_DEVICE_NAME);
        ...
        private void ConnectionFailed()
        {
            ...
            var msg = _handler.ObtainMessage(BluetoothActivity.MESSAGE_TOAST);
            Bundle bundle = new Bundle();
        ...
        private class AcceptThread : Thread
        {
            ...
            private BluetoothActivityService _service;
            ...
            public override void Run()
            {
                ...
                while (_service._state != BluetoothActivityService.STATE_CONNECTED)
                {
                    try
                    {
            ...
            public override void Run()
            {
                ...
                catch (Java.IO.IOException e)
                {
                    try
                    {
                        ...
                        //var socket = (BluetoothSocket)device.getClass().getMethod("createRfcommSocket", new Class[] { int.class}).invoke(device,1);
                        //socket.connect();
                        //mmSocket = mmDevice.Class.GetMethod("createRfcommSocketToServiceRecord ", new Class[] { Java.Lang.Integer.Type }).Invoke(mmDevice, 1);
                        mmSocket = (BluetoothSocket)mmDevice.Class.GetMethod("createRfcommSocket", new Class[] { Java.Lang.Integer.Type }).Invoke(mmDevice, 1);
                        //mmSocket = mmDevice.CreateInsecureRfcommSocketToServiceRecord(MY_UUID);
                        mmSocket.Connect();
                    }
                    catch (Exception e2)
                    {
                        Log.Error(TAG, "Couldn't establish Bluetooth connection!");
                        ...
                        {
                            Log.Error(TAG, "unable to close() socket during connection failure", e3);
                        }
        ...
        private class ConnectedThread : Thread
        {
            ...
            public override void Run()
            {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        _service._handler.ObtainMessage(BluetoothActivity.MESSAGE_READ, bytes, -1, buffer)
                            .SendToTarget();
                    }
                    catch (Java.IO.IOException e)
                    {
            ...
            public void Write(byte[] buffer)
            {
                try
                {
                    ...
                    _service._handler.ObtainMessage(BluetoothActivity.MESSAGE_WRITE, -1, -1, buffer)
                        .SendToTarget();
                }
                catch (Java.IO.IOException e)
                {
